// Ethical Area Chart - Assignment 10
...
//   1. Y-axis starts at 0 shows true proportion of weekly accumulation
//   2. Neutral distinct colors encode group, not alarm
//   3. Accurate descriptive title no editorial framing
...
async function draw() {
  ...
  // Correction #2: neutral distinct colors teal, blue, gray no alarm implied
  const targetGroups = ["18–20", "40–44", "65+"];
  const colors   = { "18–20": "#1D9E75", "40–44": "#378ADD", "65+": "#888780" };
  const opacities = { "18–20": 0.55,     "40–44": 0.45,      "65+": 0.40     };
  ...
  // SVG
  const svg = d3.select("#a10-areachart-ethical")
    .append("svg")
    .attr("width",  width)
    .attr("height", height);

  // Correction #3: accurate neutral title
  svg.append("text")
    ...
    .style("fill", "#333")
    .text("Cumulative Weekly Social Media Usage by Age Group (2024)");
  ...
  svg.append("text")
    ...
    .text("Running total of minutes per week · hover for details · Source: Statista/GWI, Feb 2024");
  ...
  // Correction #1: y-axis starts at 0 shows true scale of accumulation
